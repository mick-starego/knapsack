...
function run(target, tax, itemList) {
    ...
    // Add tax and total
    const subtotal = bestResult.reduce((prev, curr) => prev + curr.totalPrice, 0);
...
function getScore(result, calculatedItems, calculatedTarget) {
    ...
    // Cost efficiency: Total number of items included
    const numItemsIncluded = result.reduce((prev, curr) => prev + curr.quantity, 0);
    const minItemPrice = Math.min(...calculatedItems.map((item) => item.integerPrice));
    const maxItemsIncluded = Math.floor(calculatedTarget / minItemPrice);
    const efficiencyScore = numItemsIncluded / maxItemsIncluded;
